...
def run(config_path: str) -> dict:
    ...
    with BinanceFuturesClient() as client:
        ...
        exchange_path = ensure_parent(
            DATA_ROOT / "raw" / "binance" / "exchange_info" / "exchange_info.json"
        )
        exchange_path.write_text(
            __import__("json").dumps(exchange_info, ensure_ascii=False, indent=2), encoding="utf-8"
        )
