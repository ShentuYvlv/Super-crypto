...
def get_overview():
    ...
    latest_experiment = next(iter(sorted(experiments, key=lambda item: item["created_at"], reverse=True)), None)
    latest_pipeline_id = latest_pipeline["run_id"] if latest_pipeline else None
    latest_enrich_stage = latest_pipeline_stage(latest_pipeline_id, "enrich") if latest_pipeline_id else None
    latest_score_stage = latest_pipeline_stage(latest_pipeline_id, "score_symbols") if latest_pipeline_id else None
    latest_trades = [
        trade
        for trade in trades
    ...
    if latest_enrich_stage:
        ...
        if any(value == "missing_api_key" for symbol_info in coinglass.values() for value in symbol_info.values()):
    ...
    payload = {
        ...
        "validation_best_net_return": latest_experiment["metrics"]["net_return"] if latest_experiment else 0.0,
